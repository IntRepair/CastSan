...
static bool sd_isVtableDecl(const clang::BaseSubobject * Base, clang::CodeGen::CodeGenModule * CGM, const clang::CXXRecordDecl * RD) {

  clang::CodeGen::CGCXXABI *ABI = &CGM->getCXXABI();
  std::string Name = sd_getClassName(ABI, RD, Base);
  std::string Name2 = sd_getClassName(ABI, RD, Base);
  ...
  while(!Layout->hasOwnVFPtr()) {
	...
	if(!sd_isVtableName(Name))
		return false;
	//std::cerr << "Class " << Name2 << " is subclassing from " << Name << std::endl;
	//assert(sd_isVtableName(Name) && "There is subclassing of std");
  }
  return true;
}
                              ...
                              const clang::BaseSubobject *Base = NULL)
{
  ...
  if (!sd_isVtableDecl(Base, CGM, RD))
  {
    return;
  }
